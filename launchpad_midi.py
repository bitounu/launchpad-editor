...
"""Launchpad Mini MK3 – CLI tool using pure-Python MIDI (no amidi).

Drop-in replacement for launchpad_grid.py that uses the ``mido`` library
with the ``python-rtmidi`` backend instead of shelling out to ``amidi``.
"""
...
def build_frame_sysex(frame: Frame) -> list[int]:
    """Return SysEx payload as a list of ints (without F0/F7 framing)."""
    data = [0x00, 0x20, 0x29, 0x02, 0x0D, 0x03]
    for row_index, row in enumerate(frame.rows):
        for col_index, token in enumerate(row):
            data.extend([0x00, GRID_INDICES[row_index][col_index], color_value(token)])
    for index in FUNCTION_INDICES:
        data.extend([0x00, index, 0x00])
    return data


def build_clear_sysex() -> list[int]:
    return build_frame_sysex(Frame(rows=[["."] * 8 for _ in range(8)], delay_ms=0))


def build_programmer_mode_sysex() -> list[int]:
    return [0x00, 0x20, 0x29, 0x02, 0x0D, 0x00, 0x7F]
...
def build_text_scroll_sysex(text: str, *, loop: bool = False,
                            speed: int = 10, color_bytes: list[int] | None = None) -> list[int]:
    data = [0x00, 0x20, 0x29, 0x02, 0x0D, 0x07]
    data.append(1 if loop else 0)
    if speed < 0:
        speed_byte = (abs(speed) + 0x80) & 0x7F
    else:
        speed_byte = speed & 0x3F
    data.append(speed_byte)
    if color_bytes is None:
        color_bytes = [0x00, 0x03]
    data.extend(color_bytes)
    data.extend(ord(c) for c in text)
    return data


def build_text_stop_sysex() -> list[int]:
    return [0x00, 0x20, 0x29, 0x02, 0x0D, 0x07]


def build_grid_sysex(color_indices: list[int]) -> list[int]:
    ...
    for row in range(8):
        for col in range(8):
            ...
            data.extend([0x00, GRID_INDICES[row][col], color])
    for fi in FUNCTION_INDICES:
        data.extend([0x00, fi, 0x00])
    return data


# ── MIDI port management (mido / python-rtmidi) ─────────────────────

def send_sysex(port: mido.ports.BaseOutput, data: list[int]) -> None:
    """Send a SysEx message via an open mido output port."""
...
def auto_detect_port_name() -> str:
    ...
    if not outputs:
        raise ValueError("No MIDI output ports found.")
    if len(outputs) == 1:
        print(f"No Launchpad port found; using the only available MIDI output: {outputs[0]}")
        return outputs[0]
    available = ", ".join(outputs)
...
def resolve_port_name(args: argparse.Namespace) -> str:
    if args.port:
        return args.port
    return auto_detect_port_name()


def open_output(port_name: str) -> mido.ports.BaseOutput:
...
class LiveHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            send_sysex(self.midi_out, sysex)
...
def run_serve(args: argparse.Namespace) -> int:
    ...
    send_sysex(midi_out, build_programmer_mode_sysex())
    send_sysex(midi_out, build_clear_sysex())

    LiveHandler.midi_out = midi_out
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and scenes on the hardware. Uses the mido library for MIDI output "
            "(no amidi required). Can also serve as a live preview bridge "
    ...
    parser.add_argument("-p", "--port", metavar="PORT",
                        help="MIDI port name (e.g. 'Launchpad Mini MK3:Launchpad Mini MK3 MIDI 1 20:0'); auto-detected if omitted")
    ...
    try:
        ...
        if args.scene:
            ...
            for _ in range(loops):
                ...
                send_sysex(midi_out, build_frame_sysex(frame))
        ...
        if args.frame is not None:
            ...
            send_sysex(midi_out, build_frame_sysex(frames[index]))
            return 0
    ...
    try:
        if args.list_ports:
            ...
            if not outputs:
                ...
                for name in outputs:
                    lp = " [Launchpad]" if is_launchpad_port(name) else ""
                    print(f"  {name}{lp}")
            return 0
        if args.serve:
        ...
        if args.text_stop:
            port_name = resolve_port_name(args)
            midi_out = open_output(port_name)
            send_sysex(midi_out, build_text_stop_sysex())
            midi_out.close()
            return 0
